...
{
    public class Aria2ClientAddMetalinkTests
    {
        ...
        public Aria2ClientAddMetalinkTests()
        {
            _target = new Aria2Client(_requestHandler, _notificationHandler);
        }

        [Fact]
        public async Task GIVEN_ValidMetalink_WHEN_AddMetalink_THEN_ShouldPassAddMetalinkRequestToHandler()
        {
            var expected = "result";
            var response = new JsonRpcResponse<string> { Result = expected, Error = null, Id = "Id", JsonRpc = "JsonRpc" };
            var metalink = "MetalinkData";

            Mock.Get(_requestHandler)
                .Setup(x => x.SendRequest<string>(It.IsAny<JsonRpcRequest>()))
                .ReturnsAsync(response);

            var result = await _target.AddMetalink(metalink);

            result.Should().Be(expected);

            Mock.Get(_requestHandler)
                .Verify(x => x.SendRequest<string>(It.Is<AddMetalink>(r => r != null)), Times.Once());
        }
    }
}
